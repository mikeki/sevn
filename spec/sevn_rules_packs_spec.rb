...
describe Sevn do
  ...
  describe "Rules Packs" do
    ...
    describe "<<" do
      ...
      it { lambda { abilities << nil }.should raise_error(Sevn::InvalidPackPassed) }
    ...
    describe :add! do
      it { abilities.add!(:global, rules).should be_true }
      it { lambda { abilities.add!(:wrong, nil)}.should raise_error(Sevn::InvalidPackPassed) }
    end
    ...
    describe "namespace(pack) usage" do
      ...
      describe :use! do
        ...
        it "should raise error if trying to use unexisting pack" do
          lambda { abilities.use!(:noname)}.should raise_error(Sevn::NoPackError)
        end
      end
    end
    ...
    context "removing pack" do
      ...
      describe :remove! do
        it { abilities.remove!(:global).should be_true }
        it { lambda { abilities.remove!(:zzz)}.should raise_error(Sevn::NoPackError) }
      end
    end
